...
template<typename REAL>
std::shared_ptr<void> ServiceFactory::create(ServiceType serviceType,
		std::shared_ptr<DataManager> dataMng, std::vector<int> const& device_ids)
{
#ifndef CUDA
	(void) args;
#endif
...
#ifdef CUDA
		case ServiceType::GPUEnergyService6D:
			return std::shared_ptr<void>( new GPUEnergyService6D<REAL>(dataMng, device_ids));
		case ServiceType::GPUEnergyService6DModes:
			return std::shared_ptr<void>( new GPUEnergyService6DModes<REAL>(dataMng, device_ids));
#endif
		default:
			throw std::invalid_argument("unknown app to create: " + static_cast<int>(serviceType));
	}
